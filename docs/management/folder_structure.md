...
**実施しない**。全て1行の抽象定義で中身が薄く、`I` プレフィックスでソートされているため
フラットのままでも探すのに困らない。分割しても得られる一覧性の向上に対して、
include パスの変更量が見合わない。

インターフェースが増えて実際に探しにくくなった時点で再検討する。
