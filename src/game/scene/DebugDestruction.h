...
namespace game::scene
{
	 ...
	 * 実機で確かめるための使い捨てシーン。プロトタイプは docs/prototype/destruction.html。
	 *
	 * 無傷のあいだは1個のブロックを描き、ひびはテクスチャの差し替えで進める。
	 * 破壊した瞬間に、あらかじめ割ってあるモデル（gen_fracture_models.py の出力）へ
	 * 差し替え、フレームごとに座標変換を差し込んで破片を飛ばす。
	 *
	 * @note ECS を通さずシーン内で完結させている。検証が済んだら破棄する前提であり、
	 *       Component/System を作ると本体側に消し忘れが残るため
	 * @note 破片は1体のモデルのフレームなので、何個あっても描画は1回で済む
	 */
