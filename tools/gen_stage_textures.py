...
def mqo_text(tex_filename):
    ...
    lines.append("\t}")
    # 両面にするため各面を表裏2枚ずつ出す
    lines.append("\tface %d {" % (len(FACES) * 2))
    for f in FACES:
        uvflat = " ".join("%.4f %.4f" % (UV[i][0], UV[i][1]) for i in range(4))
        lines.append("\t\t4 V(%d %d %d %d) M(0) UV(%s)" % (f[0], f[1], f[2], f[3], uvflat))
        rf = (f[3], f[2], f[1], f[0])
        ruv = " ".join("%.4f %.4f" % (UV[3 - i][0], UV[3 - i][1]) for i in range(4))
        lines.append("\t\t4 V(%d %d %d %d) M(0) UV(%s)" % (rf[0], rf[1], rf[2], rf[3], ruv))
    lines.append("\t}")
    lines.append("}")
    return "\n".join(lines) + "\n"
